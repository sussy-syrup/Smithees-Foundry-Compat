...
public class SmitheeFoundryReiClient implements REIClientPlugin {
    ...
    public final static CategoryIdentifier<MeltDisplay> MELTING = CategoryIdentifier.of(
            Main.MODID,
            "melting"
    );
    ...
    {
        Map<Item, SmelteryResource> map = ApiSmelteryResourceRegistry.getInstance().getSmelteryResourceMap();
        ...
        for(Item item : map.keySet())
        {
            ...
            SmelteryResource resource = map.get(item);

            FluidProperties fluidProp = ApiMoltenFluidRegistry.getInstance().getFluidProperties(resource.fluidID());
